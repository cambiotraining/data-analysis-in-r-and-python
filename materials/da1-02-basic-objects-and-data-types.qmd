...
We can create a number sequence using `seq()`. We specify the starting number, the end number and the step-size.
...
We can create a number sequence using `range()`. We specify the starting number, the end number and the step-size.
...
### Summation {#sec-exr_summation}

::::: {.callout-exercise #ex-summation}
#### Summation
...
We can create a number sequence using `seq()`. We specify the starting number, the end number and the step-size. Because we're after uneven numbers, we start at `11`.
...
We can create a number sequence using `range`()`. We specify the starting number, the end number and the step-size. 
Because we're after uneven numbers, we start at `11`.
```{python}
...
### Data subsetting {#sec-exr_data_subsetting}

::::: {.callout-exercise #ex-data_subsetting}
#### Data subsetting

{{< level 2 >}}

Create an object that contains the following values:

```bash
"Genetics", "Geography", "Neurosurgery", "Medicine", "Zoology", "Chemistry"
```

Extract the values where the number of characters in the Department name is more than 8.
...
Use `nchar()` to count the number of characters.

## Python

Use `np.char.str_len()` to count the number of characters.
...
We need to create an object to contain the data and then extract only the entries that are longer than 8 characters.

::: {.panel-tabset group="language"}
## R

```{r}
departments <- c("Genetics", "Geography", "Neurosurgery", "Medicine", "Zoology", "Chemistry")
```

```{r}
# Subset elements where the length of each string is greater than 8
departments[nchar(departments) > 8]
```
...
```{python}
departments = np.array(["Genetics", "Geography", "Neurosurgery", "Medicine", "Zoology", "Chemistry"])
```

```{python}
# Subset elements where the length of each string is greater than 8
departments[np.char.str_len(departments) > 8]
```
...
### Data types {#sec-exr_data_types}

::::: {.callout-exercise #ex-data_types}
#### Data types

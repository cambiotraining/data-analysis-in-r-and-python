...
The default Excel format is not great, since it's a propriety format and not natively readable by other computer programmes. Good alternatives are `.csv` or *comma-separated values* files or `.tsv` *tab-separated values* files.

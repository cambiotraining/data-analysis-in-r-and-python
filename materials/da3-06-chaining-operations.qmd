...
you are referring to the `weight` column in the **original `surveys` DataFrame**. But sometimes you want to use a column that was **created earlier in the method chain**. In that case, the column doesn't exist in the original DataFrame.

Using `lambda` lets `.assign()` work with the DataFrame **as it exists at that point in the chain**.

For example, suppose we want to:

- Select `record_id` and `weight`
- Create `weight_kg`
- Create another column using the newly created `weight_kg`
...
(
  ...
  .assign(weight_kg = surveys["weight"] / 1000)
  .assign(weight_doubled = surveys["weight_kg"] * 2)
)
```

This fails because `weight_kg` does **not** exist in the original `surveys` DataFrame. With `lambda`, we can refer to the DataFrame at the current point in the chain:
...
(
  ...
  .assign(weight_doubled = lambda df: df["weight_kg"] * 2)
)
```

Here, `df["weight_kg"]` works because `df` represents the DataFrame **after `weight_kg` has been created**.

**Key thing to remember:** use `lambda` when your calculation needs to refer to columns that were created or modified earlier in the method chain.
:::
...
We use `select()` to select the columns, `rename()` to rename the `fish_length` column and `mutate()` to create a new column containing the fishlength values in inches.
...
We use `.loc` to select the columns, `.rename()` to rename the `fish_length` column and `.assign()` to create the new column containin the fishlength values in inches.
...
### Converting dates: `surveys` {#sec-exr_converting_dates}

::::: {.callout-exercise #ex-converting_dates}
#### Converting dates
...
1. `record_date` that contains the full date in the format `yyyy-mm-dd`
2. `month_abbr` that contains the month abbreviation (e.g. `Jan`, `Feb`, ...)
3. Do this using chains, so you don't end up needing to save lots of intermediate objects

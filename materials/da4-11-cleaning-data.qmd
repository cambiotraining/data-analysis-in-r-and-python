...
In Python, `plotnine` supports plot composition natively. Use `|` to place plots side by side and `/` to stack them vertically. For more information, see [here](https://plotnine.org/reference/Compose.html#plotnine.composition.Compose).
...
We can add tags to the individual plots using `labs(tag = ...)`. We can use Quarto's `fig-cap` option to label the full figure:
...
composition = (p1_tagged | p2_tagged) / p3_tagged
composition
```

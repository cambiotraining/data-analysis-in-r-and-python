...
```{python}
#| results: hide
#| label: fig-weight_hind_scatter_pandas
#| fig-cap: "Scatterplot of weight vs hindfoot_length"
...
#| fig-cap: "Seaborn uses `color` for fill colours."
...
#| warning: false
#| label: fig-facetgrid
#| fig-cap: "Splitting into rows and columns."
ggplot(data = surveys,
       mapping = aes(x = weight, y = hindfoot_length)) +
  geom_point() +
...
Which lake has the highest parasite count value? Eyeball this using the plots!

::: {.callout-answer collapse="true"}
We'll assume you still have the data loaded. We create the scatterplot, then facet it by `lake`.
...
### Customising visuals: `parasites` {#sec-exr_custom_visuals}

::::: {.callout-exercise #ex-custom_colours}
#### Customising visuals
...
The goal of this exercise is to become more familiar with some of the customisations that are possible. You'll be focusing on adding titles and custom axis labels to a plot. You'll also practice customising the colour scheme of a plot.
...
You can refer to the [Clean, style & arrange](da4-11-cleaning-data.qmd) chapter for suggestions on how to do this!

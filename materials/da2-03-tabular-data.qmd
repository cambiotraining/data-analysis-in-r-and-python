...
1. Load the data.
2. Check the structure of the data. What column types do we have?
3. Select the `lake` column. How many lakes are there?
4. Select rows 12 to 29 of the `fish_length` column. Using code, what are the minimum and maximum values?
...
We can define the column explicitly, using the `["column_name"]` notation.
...
### Random sampling: `parasites` {#sec-exr_random_sampling}

::::: {.callout-exercise #ex-random_sampling}
#### Random sampling
...
1. The data (`parasites`)
2. The number of samples to take (`n = `)
3. (optional) setting the random seed (`random_state = `)
